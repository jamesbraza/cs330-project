...
def embedding_resnet(X_PATH, Y_PATH):
    ...
    output = tf.keras.applications.ResNet50V2(
    ...
    embedding_resnet_r = tf.reshape(
        embedding_resnet, shape=(embedding_resnet.shape[0], -1)
    )
...
if __name__ == "__main__":
    ...
    x_tune = np.load(
        "/content/drive/My Drive/cs330_dataset/x_tune_64_all_embedding_class_mean.npy"
    )
    class_mean_x = embedding_resnet(X_PATH, Y_PATH)
    out = tf.concat([class_mean_x, x_tune], axis=0)
    print(out.shape)
    np.save(
        "/content/drive/My Drive/cs330_dataset/x1_64_all_embedding_class_mean.npy", out
    )
